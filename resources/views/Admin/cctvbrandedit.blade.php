                ...
                <div class="col-sm-12">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-sm-5">
                                    <form action="{{url('admin/cctv-brand-update',[$BrandDetail->id])}}" method="POST">
                                        @csrf
                                        <div class="form-group">
                                            <label for="BrandName">CCTV Brand Name</label>
                                            <input type="text" class="form-control{{$errors->has('brandDisplayName') ? ' is-invalid' : ''}}" value="{{$BrandDetail->brandDisplayName}}" id="brandDisplayName"  name="brandDisplayName" placeholder="Enter CCTV Brand Name">
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select id="brandID" class="form-control {{$errors->has('brandID') ? ' is-invalid' : ''}}" id="brandID" name="brandID">
                                                ...
                                                @foreach(App\ProductsBrand::all() as $ProductcBrand)
                                                    <option value="{{$ProductcBrand->id}}" {{$ProductcBrand->id == $BrandDetail->brandID ? 'selected="selected"' : ''}}>{{$ProductcBrand->BrandName}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <button type="submit" class="btn btn-primary">Save CCTV Update</button>
                                    </form>
                                </div>
                                <div class="col-sm-7">
