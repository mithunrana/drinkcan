        ...
        <section id="app" style="margin-top: 10px;" class="content">
            <div class="row">
                <div class="col-sm-12">
                    ...
                    @endif
                    <div class="card">
                        <div class="card-header">Store Location Edit</div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-sm-7">
                                    <form action="{{url('admin/store-location-update',[$StoreLocation->id])}}" method="POST">
                                        @csrf
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="ThanaId" required>
                                                @foreach(\App\ThanaName::get() as $Thana)
                                                    <option value="{{$Thana->id}}" {{$Thana->id == $StoreLocation->id ? 'selected="selected"' : ''}}>{{$Thana->Name}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label for="LocationName">Store Location Name</label>
                                            <input type="text" class="form-control{{$errors->has('LocationName') ? ' is-invalid' : ''}}" value="{{$StoreLocation->LocationName}}" id="LocationName"  name="LocationName" placeholder="Enter Location Name" required>
                                        </div>
                                        <div class="form-group">
                                            <label for="FullAddress">Store Location Address</label>
                                            <textarea type="text" class="form-control{{$errors->has('FullAddress') ? ' is-invalid' : ''}}"  id="FullAddress" name="FullAddress" placeholder="Enter FullAddress...." required>{{$StoreLocation->FullAddress}}</textarea>
                                        </div>
