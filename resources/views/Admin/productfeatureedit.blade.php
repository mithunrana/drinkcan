        ...
        <section id="app" style="margin-top: 10px;" class="content">
            <div class="row">
                <div class="col-sm-12">
                    ...
                    @endif
                    <div class="card">
                        <div class="card-header">Product Feature Added Panel</div>
                        <div class="card-body">
                            <div class="row">
                                <div id="app" class="col-sm-6">
                                    ...
                                    <form action="{{url('admin/product-feature-update',[$Feature->id])}}" method="POST">
                                        @csrf
                                        <div class="row">
                                            <div class="col-sm-6">
                                                ...
                                                @endif
                                                <img id="previewImage" style="width: 100%;margin-top: 27px;" src="/{{$Feature->featuredimage->imageurl}}">
                                            ...
                                            <div class="form-group col-sm-6">
                                                ...
                                                <select class="form-control{{$errors->has('ProductId') ? ' is-invalid' : ''}}" id="ProductId" name="ProductId">
                                                    ...
                                                    @foreach(App\Products::all() as $Product)
                                                        <option value="{{$Product->id}}" {{$Product->id == $Feature->ProductId ? 'selected="selected"' : ''}}>{{$Product->Model}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control{{$errors->has('Title') ? ' is-invalid' : ''}}" value="{{$Feature->Title}}" id="Title"  name="Title" placeholder="Enter Title ">
                                        </div>
                                        <div class="form-group">
                                            <label for="Content">Details</label>
                                            <textarea type="text" rows="7" class="form-control{{$errors->has('Content') ? ' is-invalid' : ''}}"  id="Content" name="Content" placeholder="Enter Content Details....">{{$Feature->Content}}</textarea>
                                        </div>

                                        <input type="hidden" name="FeaturedImage" id="setimageid" value="{{$Feature->FeaturedImage}}">
                                        <input type="hidden" name="ImageAltText" id="setImageAltText"  value="{{$Feature->ImageAltText}}">
                                        <input type="hidden" name="ImageTitleText" id="setImageTitleText" value="{{$Feature->ImageTitleText}}">
...
<script>
    ...
    $(document).ready( function () {
        ...
        $('#featuredimage').click(function(){
            $('#previousimage').attr('src','/'+'{{$Feature->featuredimage->imageurl}}');
            $('#imagelocation').attr('value','{{$Feature->featuredimage->imageurl}}');
            $('#getimageId').attr('value','{{$Feature->FeaturedImage}}');
            $('#getImageAltText').attr('value','{{$Feature->ImageAltText}}');
            $('#getImageTitleText').attr('value','{{$Feature->ImageTitleText}}');
        });
